...
func (t *Planner) findBestWindowCombination(windows []chargingWindow, requiredDuration time.Duration) *planCandidate {
	...
	// Find the best single window solution (if any)
	var bestSingleCandidate *planCandidate
	for i := range windows {
		w := &windows[i]
		...
		if w.duration >= requiredDuration {
			...
			if candidate != nil && (bestSingleCandidate == nil || candidate.score < bestSingleCandidate.score) {
				bestSingleCandidate = candidate
			}
		}
	}

	// Also find the best multi-window solution using greedy selection
	...
	var bestMultiCandidate *planCandidate
	if len(selected) > 0 {
		bestMultiCandidate = t.evaluateWindowCombination(selected, requiredDuration)
	}

	// Return the best solution overall (single or multi-window)
	if bestSingleCandidate != nil && bestMultiCandidate != nil {
		if bestSingleCandidate.score <= bestMultiCandidate.score {
			return bestSingleCandidate
		}
		return bestMultiCandidate
	}
	if bestSingleCandidate != nil {
		return bestSingleCandidate
	}
	return bestMultiCandidate
}
