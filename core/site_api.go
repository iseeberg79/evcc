...
func (site *Site) SetBatteryEstimator(val bool) error {
	...
	if site.batteryEstimator != val {
		site.batteryEstimator = val
		settings.SetBool(keys.BatteryEstimator, val)
		site.publish(keys.BatteryEstimator, val)
	}
...
func (site *Site) SetBatteryEstimatorFactor(val float64) error {
	...
	if site.batteryEstimatorFactor != val {
		site.batteryEstimatorFactor = val
		settings.SetFloat(keys.BatteryEstimatorFactor, val)
		site.publish(keys.BatteryEstimatorFactor, val)
	}
...
func (site *Site) GetBatteryEstimatorTargetTime() string {
	site.RLock()
	defer site.RUnlock()
	return site.batteryEstimatorTargetTime
}

// SetBatteryEstimatorTargetTime sets the target time (HH:MM) by which the battery
// estimator aims to have the battery full
func (site *Site) SetBatteryEstimatorTargetTime(val string) error {
	site.log.DEBUG.Println("set battery estimator target time:", val)
	...
	if site.batteryEstimatorTargetTime != val {
		site.batteryEstimatorTargetTime = val
		settings.SetString(keys.BatteryEstimatorTargetTime, val)
		site.publish(keys.BatteryEstimatorTargetTime, val)
	}
