...
func TestHoldChargeMode(t *testing.T) {
	for _, tc := range []struct {
		...
		{"empty action (default, same path as normal)", map[string]types.Suggestion{"b": {}}, api.BatteryNormal},
...
// TestHoldChargeYieldsToSmartChargeSession guards that Hold wins over HoldCharge for the
// whole duration of a smart-cost/fast charge session, even when the loadpoint status
// briefly drops from C to B (PWM pause, phase switch, handshake retry).
...
func TestHoldChargePlanSlotLookup(t *testing.T) {
	...
	t.Run("holdChargePlanAvailable", func(t *testing.T) {
		require.False(t, (&Site{}).holdChargePlanAvailable(), "no plan")

		fresh := &Site{holdChargePlan: singleSlotHoldChargePlan(time.Now(), map[string]types.Suggestion{"b": {Charge: 1000}})}
		require.True(t, fresh.holdChargePlanAvailable(), "fresh plan")

		stalePlan := singleSlotHoldChargePlan(time.Now(), map[string]types.Suggestion{"b": {Charge: 1000}})
		stalePlan.updated = time.Now().Add(-2 * holdChargeStale)
		require.False(t, (&Site{holdChargePlan: stalePlan}).holdChargePlanAvailable(), "stale plan")

		// recently updated, but its own slots don't reach into now - e.g. the plan was
		// built from a stale optimizer response covering a past time window
		past := time.Now().Add(-time.Hour)
		noSlotForNow := &holdChargePlan{
			updated: time.Now(),
			starts:  []time.Time{past},
			ends:    []time.Time{past.Add(15 * time.Minute)},
			slots:   []map[string]types.Suggestion{{"b": {Charge: 1000}}},
		}
		require.False(t, (&Site{holdChargePlan: noSlotForNow}).holdChargePlanAvailable(), "fresh updated timestamp, but no slot covers now")
	})
}
