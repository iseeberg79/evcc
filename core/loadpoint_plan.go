...
func (lp *Loadpoint) GetPlan(targetTime time.Time, requiredDuration, precondition time.Duration, maxChargingWindows ...int) api.Rates {
	if lp.planner == nil || targetTime.IsZero() {
		return nil
	}

	return lp.planner.Plan(requiredDuration, precondition, targetTime, maxChargingWindows...)
}
